                    ...
                    <div style="margin-top:8px; display:flex; flex-direction:column; gap:6px;">
                        ${birthdayStudents.map(bs => html`
                            <div class="card" style="padding:8px 10px; display:flex; align-items:center; gap:10px; background-color: var(--color-surface);">
                                <img src="/birthday_${bs.gender === 'F' ? 'pink' : 'blue'}.png" alt="Pastel" style="width:20px; height:20px;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; gap: 8px; flex-wrap: wrap;">
                        ...
                        <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                            ...
                            ${state.showFullMap ? html`
                                ...
                                <button
                                    type="button"
                                    style="padding:6px 10px; font-size:0.8rem; background-color: #1976d2; color: #fff; border-radius:8px;"
                                    @click=${() => exportAbsenteesLatePDF(section.id)}>
                                    Exportar Inasistentes/Retardos (PDF)
                                </button>
                            ` : null}
                            <button
                                type="button"
...
/**
 * Export a PDF listing, for each date in the section's attendance records (ordered),
 * two columns: Inasistentes and Retardos. Each column lists students sorted by lastName, firstName.
 */
function exportAbsenteesLatePDF(sectionId) {
    const section = DataManager.getSection(sectionId);
    if (!section) {
        alert('Sección no encontrada.');
        return;
    ...
    // Ask whether to export just this section or all sections of the same subject
    const subject = DataManager.getSubjects().find(s => s.id === section.subjectId);
    const choice = prompt(
        `Exportar para:\n1) Esta sección (${section.name})\n2) Todas las secciones de la materia${subject ? ` (${subject.name})` : ''}\n\nEscribe 1 o 2:`,
        '1'
    );
    if (choice === null) return;
    const exportAllSections = (choice.trim() === '2');

    // Ask user for start and end date for export (YYYY-MM-DD).
    const todayStr = new Date().toISOString().split('T')[0];
    let start = prompt(`Desde (YYYY-MM-DD). Puedes elegir cualquier fecha:`, todayStr);
    if (start === null) return;
    start = start.trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(start)) {
        alert('Formato de fecha "Desde" inválido. Se usará la fecha por defecto.');
        start = todayStr;
    }

    let end = prompt(`Hasta (YYYY-MM-DD). Puedes elegir cualquier fecha:`, todayStr);
    if (end === null) return;
    end = end.trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(end)) {
        alert('Formato de fecha "Hasta" inválido. Se usará la fecha por defecto.');
        end = todayStr;
    }
    ...
    // Ask which weekdays to include: comma-separated numbers 0-6 (0=Sun) or names or 'all'
    let weekdaysInput = prompt(
        `Días de la semana a incluir (por defecto Lunes-Viernes):\n` +
        `Opciones: "all" o lista de números 0-6 separados por comas (0=Domingo), o nombres abreviados ej: lun,mar,mie\n` +
        `Ejemplos: all  |  1,2,3,4,5  |  lun,mar,mie`,
        '1,2,3,4,5'
    );
    if (weekdaysInput === null) return;
    weekdaysInput = weekdaysInput.trim().toLowerCase();

    const weekdaySet = new Set();
    if (weekdaysInput === 'all' || weekdaysInput === '') {
        [0,1,2,3,4,5,6].forEach(n => weekdaySet.add(n));
    } else if (/^[0-6,\s]+$/.test(weekdaysInput)) {
        weekdaysInput.split(',').map(s => s.trim()).forEach(tok => {
            if (tok === '') return;
            const n = parseInt(tok, 10);
            if (!isNaN(n) && n >= 0 && n <= 6) weekdaySet.add(n);
        });
    } else {
        const nameMap = { 'dom':0, 'lun':1, 'mar':2, 'mie':3, 'mié':3, 'jue':4, 'vie':5, 'sab':6 };
        const parts = weekdaysInput.split(',').map(s => s.trim().slice(0,3));
        parts.forEach(p => {
            if (nameMap.hasOwnProperty(p)) weekdaySet.add(nameMap[p]);
        });
        if (weekdaySet.size === 0) {
            [1,2,3,4,5].forEach(n => weekdaySet.add(n));
        }
    }

    // Build date list between start and end inclusive, filtered by weekdaySet.
    const datesInRange = [];
    ...
    if (datesInRange.length === 0) {
        alert('No hay días seleccionados en el rango indicado.');
        return;
    }

    // Helper to sort students by lastName, then firstName
    ...
    // Prepare sections list: either single or all sections of the subject
    let sectionsToExport = [section];
    if (exportAllSections && section.subjectId) {
        sectionsToExport = DataManager.getSections().filter(s => s.subjectId === section.subjectId);
    ...
    const margin = 40;
    const pageWidth = 595; // A4 pt width approx
    const colGap = 20;
    const colWidth = (pageWidth - margin * 2 - colGap) / 2;
    let y = 40;

    // Document header
    const titleBase = exportAllSections && subject ? `Inasistentes y Retardos - ${subject.name}` : `Inasistentes y Retardos - ${section.name}`;
    doc.setFontSize(14);
    doc.text(titleBase, margin, y);
    y += 18;
    doc.setFontSize(10);
    doc.text(`Período: ${start} - ${end}`, margin, y);
    y += 14;

    // For each section, print a heading and then iterate dates
    sectionsToExport.forEach((sec, secIndex) => {
        if (y > 740) { doc.addPage(); y = 40; }
        doc.setFontSize(12);
        doc.text(`Sección: ${sec.name}`, margin, y);
        y += 14;

        const attendanceRecords = DataManager.getAttendanceRecords(sec.id);

        // Determine the section's visible weekdays (1=Mon...5=Fri). Map to 0-6 scale used above.
        const secVisibleWeekdays = Array.isArray(sec.visibleWeekdays) && sec.visibleWeekdays.length
            ? sec.visibleWeekdays.slice()
            : [1,2,3,4,5];
        const secVisibleSet = new Set(secVisibleWeekdays.map(n => {
            // Convert 1..5 (Mon..Fri) to 1..5 in getUTCDay() (0=Sun)
            return Number(n);
        }));

        // Filter datesInRange to those that match both the user's weekday selection and the section's visible weekdays.
        const filteredDates = datesInRange.filter(d => {
            const dt = new Date(d + 'T00:00:00Z');
            const dow = dt.getUTCDay(); // 0..6
            // Convert dow to 0..6 and check: secVisibleWeekdays uses 1=Mon..5=Fri so accept only when dow in secVisibleSet
            // Note: secVisibleSet currently holds numbers 1..5 corresponding to Mon..Fri. So accept if dow is in that set.
            return secVisibleSet.has(dow);
        });

        // For each date, skip if there are no absentees AND no lates (omit days without registros).
        const finalDates = filteredDates.filter(dateStr => {
            const dayRecords = attendanceRecords[dateStr] || {};
            const students = DataManager.getStudents(sec.id) || [];
            for (let i = 0; i < students.length; i++) {
                const status = dayRecords[students[i].id] || 'U';
                if (status === 'A' || status === 'L') return true; // keep date if any A or L
            }
            return false; // omit date if no A/L found
        });

        if (finalDates.length === 0) {
            // If nothing to print for this section, note it and continue
            doc.setFontSize(10);
            doc.text('No hay registros de inasistencias/retardos en las fechas visibles seleccionadas.', margin, y);
            y += 14;
        } else {
            finalDates.forEach((dateStr) => {
                if (y > 740) { doc.addPage(); y = 40; doc.setFontSize(12); doc.text(`Sección: ${sec.name} (cont.)`, margin, y); y += 16; }
                doc.setFontSize(11);
                ...
                students.forEach(s => {
                    const status = dayRecords[s.id] || 'U';
                    if (status === 'A') absentees.push(s);
                    if (status === 'L') lates.push(s);
                });

                absentees.sort(sortByName);
                lates.sort(sortByName);

                const maxLines = Math.max(absentees.length, lates.length, 1);
                const lineHeight = 12;
                doc.setFontSize(10);
                doc.text('Inasistentes', margin, y);
                doc.text('Retardos', margin + colWidth + colGap, y);
                y += lineHeight;

                // At this point we know there is at least one A or L; print rows
                for (let i = 0; i < maxLines; i++) {
                    if (y > 740) { doc.addPage(); y = 40; doc.setFontSize(11); doc.text(`${dateStr} (cont.)`, margin, y); y += 16; doc.setFontSize(10); }
                    const a = absentees[i];
                    const l = lates[i];
                    if (a) {
                        const nameA = `${a.lastName}, ${a.firstName}`;
                        doc.text(nameA, margin, y, { maxWidth: colWidth });
                    }
                    if (l) {
                        const nameL = `${l.lastName}, ${l.firstName}`;
                        doc.text(nameL, margin + colWidth + colGap, y, { maxWidth: colWidth });
                    }
                    y += lineHeight;
                }
                y += 8;
        ...
        if (secIndex < sectionsToExport.length - 1) {
            y += 8;
    ...
    // Filename
    const safeName = (exportAllSections && subject) ? subject.name.replace(/\s+/g,'_') : section.name.replace(/\s+/g,'_');
    const weekdayHint = Array.from(weekdaySet).sort().join('');
    doc.save(`inasistentes_retardos_${safeName}_${start}_to_${end}_wd${weekdayHint}.pdf`);
}
...
/**
 * Send a push-style notification (Web Notifications API) and mark that we've notified
 * for this section for today's date. Only notifies once per section per day.
 */
async function checkBirthdayAlerts() {
    ...
    const names = birthdaysToday.map(s => `${s.lastName}, ${s.firstName}`).join('; ');
    const title = `Cumpleaños en ${section.name}`;
    const body = birthdaysToday.length === 1
        ? `Hoy cumple: ${names}`
        : `Hoy cumplen: ${names}`;

    // Try to request permission if needed and show notification
    try {
        if ('Notification' in window) {
            if (Notification.permission === 'granted') {
                new Notification(title, { body, icon: '/birthday_blue.png', vibrate: [100,50,100] });
            } else if (Notification.permission !== 'denied') {
                const permission = await Notification.requestPermission();
                if (permission === 'granted') {
                    new Notification(title, { body, icon: '/birthday_blue.png', vibrate: [100,50,100] });
                } else {
                    // If denied, fallback to a visual in-app console log only
                    console.log('Notification permission denied by user.');
                }
            } else {
                // permission denied previously; do nothing but mark as handled
                console.log('Notification permission previously denied.');
            }
        } else {
            // Notifications not supported; fallback log only
    ...
    // Mark as handled so we don't notify repeatedly
    state.birthdayAlarmPlayed[state.selectedSectionId] = todayStr;

    // Optional: log for debugging
    console.log(`Birthday notification sent/handled for section ${section.name} on ${todayStr}.`);
}
